package com.xappie.customviews;
 ...
 */
public class CirclePageIndicatorForTour extends View implements PageIndicator {
	...
	public CirclePageIndicatorForTour(Context context) {
		this(context, null);
	}

	public CirclePageIndicatorForTour(Context context, AttributeSet attrs) {
	...
	public CirclePageIndicatorForTour(Context context, AttributeSet attrs,
		...
		mPaintPageFill.setColor(a.getColor(
				R.styleable.CirclePageIndicator_pageColor, Color.TRANSPARENT));
		...
		mPaintFill.setColor(a.getColor(
				R.styleable.CirclePageIndicator_fillColor, Color.WHITE));
	...
	protected void onDraw(Canvas canvas) {
		...
		final float threeRadius = mRadius * 10;
		final float shortOffset = shortPaddingBefore + mRadius;
		float longOffset = longPaddingBefore + mRadius;
		if (mCentered) {
			float width = mRadius * 2.0f; // one circle width
			if (count > 1)
				width += (count - 1) * threeRadius; // each additional circle
													// needs 2r, plus 1r of
													// padding.
			longOffset = ((longSize - longPaddingBefore - longPaddingAfter) / 2.0f)
					- (width / 2.0f);
			// we need to add one radius if we have more then one circle,
			// because the draw takes hte center point.
			// for one circle, we don't, because the midpoint (which is what
			// longOffset is right now) is already
